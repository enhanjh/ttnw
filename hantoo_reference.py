...
config_root = os.path.join(os.path.dirname(__file__), '')  # 토큰 파일이 저장될 폴더, 제3자가 찾지 어렵도록 경로 설정하시기 바랍니다.
...
def auth(appkey: str, appsecret: str, svr='my_url', product=_cfg['my_prod'], url=None):
    p = {
    ...
    if saved_token is None:  # 기존 발급 토큰 확인이 안되면 발급처리
        ...
        if rescode == 200:  # 토큰 정상 발급
            ...
            return {"access_token": my_token, "expires_in": my_expired}
        else:
            print('Get Authentification token fail!')
            return {"error": f"Failed to get token: {res.text}"}
    else:
        my_token = saved_token  # 기존 발급 토큰 확인되어 기존 토큰 사용
        return {"access_token": my_token, "expires_in": "already_valid"}
...
def reAuth(svr='prod', product=_cfg['my_prod']):
    ...
    if (n2 - _last_auth_time).seconds >= 86400:  # 유효시간 1일
        auth(appkey=_cfg['my_app'], appsecret=_cfg['my_sec'], svr=svr, product=product)
...
def getTREnv():
    # This function needs to be updated to reflect the new token management
    # For now, it will return a dummy or incomplete TRENV
    nt1 = namedtuple('KISEnv', ['my_app', 'my_sec', 'my_acct', 'my_prod', 'my_token', 'my_url'])
    return nt1(
        my_app=_cfg['my_app'],
        my_sec=_cfg['my_sec'],
        my_acct=_cfg['my_acct_stock'], # Default to stock account
        my_prod='01',
        my_token=f"Bearer {get_current_token()}" if get_current_token() else None,
        my_url=_cfg['my_url']
    )
